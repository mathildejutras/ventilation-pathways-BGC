...
def load_data_argo(streamlines, paths_dist, layer, lons_mask, lats_mask, mask, name, dens1, dens2):
	...
	print('Getting Argo data...')
 	...
 	# select the data
	wprop={'Lats layer':[], 'Lons layer':[], 'Date layer':[], 'Pressure layer':[], 'Distance layer':[], 'Doxy layer':[], 'Nitrate layer':[], 'Salinity layer':[], 'Temperature layer':[], 'pH layer':[], 'Lats column':[], 'Lons column':[], 'Date column':[], 'Pressure column':[], 'Distance column':[], 'Doxy column':[], 'Nitrate column':[], 'Salinity column':[], 'Temperature column':[], 'pH column':[] }
	for f in files:
		...
		if any(item in ['DOXY', 'NITRATE', 'PH_IN_SITU'] for item in l):
			...
			if len(imask) > 0 :
				...
				# select this data
				doxy_flat = [] ; press_flat = [] ; sal_flat = [] ; temp_flat = [] ; ph_flat = [] ; nitrate_flat = []
...
#			wprop_argo['alk '+each] = alk
			wprop_argo['alk '+each] = np.zeros(len(wprop_argo['Salinity '+each]))*np.nan ###
		print('Not calculating alkalinity because of problem with matlab engine. Uncomment to use')
		...
		for each in ['layer', 'column']:
		    ...
		    wprop_argo['DIC '+each] = list(np.ones(len(wprop_argo['Lons '+each]))*np.nan)
		...
		wprop_argo_above['alk layer'] = np.zeros(len(wprop_argo_above['Salinity layer']))*np.nan
		#alk = get_alkalinity( wprop_argo_below, 'layer' )
		#wprop_argo_below['alk layer'] = alk
		wprop_argo_below['alk layer'] = np.zeros(len(wprop_argo_below['Salinity layer']))*np.nan
...
#
		...
		for var in ['Salinity', 'Temperature', 'Nitrate', 'Doxy', 'alk']:
